...
// 20100507 euikuk.jeong@lge.com Porting LGE patch <Add retry count text> from Aloha [START_LGE]
        ...
        switch (mPinChangeState) {
			...
			Log.d("FdnSetting","attemptsRemaining of PIN :" + attemptsRemaining);
                } catch (RemoteException ex) {
                    attemptsRemaining = -1;
                }
                if (strId != 0) {
                    mButtonChangePin2.setDialogMessage(getText(msgId) + "\n" + getText(strId) + "\n" +  getText(R.string.STR_TITLETEXT_UICC_REMAINING_COMMON_SHORT) + attemptsRemaining);
                } else {
                    mButtonChangePin2.setDialogMessage(getText(msgId) + "\n" +  getText(R.string.STR_TITLETEXT_UICC_REMAINING_COMMON_SHORT) + attemptsRemaining);
                }
                break;
            case PIN_CHANGE_PUK:
                try {
                    attemptsRemaining = ITelephony.Stub.asInterface(ServiceManager.checkService("phone")).getIccPuk2RetryCount();
			Log.d("FdnSetting","attemptsRemaining of PUK :" + attemptsRemaining);
                } catch (RemoteException ex) {
                    attemptsRemaining = -1;
                }
                if (strId != 0) {
...
// LGE_FDN_SUPPORT START
        Boolean status = mPhone.getIccCard().getIccFdnEnabled();
        if (status == null) {
...
// 20100507 euikuk.jeong@lge.com Porting LGE patch <Add retry count text> from Aloha [START_LGE]
        ...
        mButtonEnableFDN.setDialogMessage(getText(R.string.enter_pin2_text) + "\n" + getText(R.string.STR_TITLETEXT_UICC_REMAINING_COMMON_SHORT) + attemptsRemaining);
...
// LGE_FDN_SUPPORT START
    /**
     * Checks current SIM state and notifies user if necessarry.
     */
    protected void checkSimDataAvailability() {
        IccCard iccCard = mPhone.getIccCard();
        IccCard.State state = iccCard.getState();
        Boolean isFdnEnabled = iccCard.getIccFdnEnabled();

        if (state != IccCard.State.READY) {
            drawSimNotReadyDialog();
        } else if (isFdnEnabled == null) {
            drawReadingSimDialog();
        } else {
            return;
        }
    }
    ...
    /**
     * Draws alert "SIM not ready" and closes the activity.
     */
    protected void drawSimNotReadyDialog() {
        mButtonEnableFDN.setEnabled(false);
        mButtonChangePin2.setEnabled(false);
        mButtonFDNList.setEnabled(false);

        Toast.makeText(this, getString(R.string.sum_fdn_sim_not_ready), Toast.LENGTH_LONG).show();
    ...
    /**
     * Draws progress dialog when data from SIM is not available.
     */
    protected void drawReadingSimDialog() {
        ProgressDialog dialog = new ProgressDialog(FdnSetting.this);
        dialog.setMessage(getString(R.string.sum_fdn_reading_from_sim));
