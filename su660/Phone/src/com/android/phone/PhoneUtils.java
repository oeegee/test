...
import android.view.Gravity;//<!--//20100916 sumi920.kim@lge.com InCall-Recording 	
...
// LGE_VIDEO_TELEPHONY START
    // Return codes from placeVideoCall()
    static final int VIDEO_CALL_STATUS_DIALED = 0;  // The number was successfully dialed
    static final int VIDEO_CALL_STATUS_DIALED_MMI = 1;  // The specified number was an MMI code
    static final int VIDEO_CALL_STATUS_FAILED = 2;  // The call failed    
// LGE_VIDEO_TELEPHONY END
    ...
    private static boolean sIsHeadsetEnabled = false; //jongik2.kim 20100813 at cmd
...
//<!--[sumi920.kim@lge.com] 2010.08.27	  LAB1_CallUI AutoCall Test [START] -->
	private static boolean bAutoCallTest  = false;
	private static long    mAutoConnectionTime = 0;
	private static boolean bAutoCallRunning = false;
	
//<!--[sumi920.kim@lge.com] 2010.08.27	  LAB1_CallUI AutoCall Test [END] -->

    //<!--//20100916 sumi920.kim@lge.com InCall-Recording [START_LGE_LAB1]	 -->
    private static boolean 	mIsSoundRecording;
    private static long 	mRecDuration;
...
//20101007 wonho.moon@lge.com <mailto:wonho.moon@lge.com>Call Silent String  [START_LGE_LAB]
	private static boolean mIsReserveCall = false;
//20101007 wonho.moon@lge.com <mailto:wonho.moon@lge.com>Call Silent String  [END_LGE_LAB]
	...
	//20101101 sumi920.kim@lge.com SKT Option menu [START_LGE_LAB1]
	//START youngmi.uhm@lge.com 2010. 9. 02. LAB1_CallUI KT IOT err msg after transfer call
	private static boolean mIsTransferCall = false;
	...
	private static boolean bHaveToSKAFSpeakerMode = false;
	private static boolean bSKAFSpeakerMode = false;
	//20101105 sumi920.kim@lge.com SKT SKAF SpeakerOn/Off setting [END_LGE_LAB1]

	//20101111 sumi920.kim@lge.com Receive HDVideoCall Enable Event From HDVideoCall App. [START_LGE_LAB1]	 -->
	private static boolean 	mHDVisibilty = false;
    /**
     ...
     */
    //@Fixed Me - add a "public" cause by using at a  VTEventHandler class. by hgkim 08/11
    ...
    static int placeCall(Phone phone, String number, Uri contactRef) {
        ...
        try {
            ...
            // 20101023 sumi920.kim@lge.com TD fix : return during Dialing [STAR_LGE_LAB1]
            // START pyung.lee@lge.com 2010.08.20 LAB1_CallUI TD32429 fix : return during Dialing
            Call.State foregroundCallState = phone.getForegroundCall().getState();
            if (DBG) log("placeCall() : foreground call state = " + foregroundCallState);
            if (foregroundCallState.isDialing()) { // if dialing
            	return status;
            }
        	// END pyung.lee@lge.com 2010.08.20 LAB1_CallUI TD32429 fix : return during Dialing
            // 20101023 sumi920.kim@lge.com TD fix : return during Dialing [END_LGE_LAB1]
...
// LGE_VIDEO_TELEPHONY START
    /**
     * Video Dial the number using the phone passed in.
     *
     * @param phone the Phone object.
     * @param number the number to be dialed.
     * @return either CALL_STATUS_DIALED, CALL_STATUS_DIALED_MMI, or CALL_STATUS_FAILED
     */ 
    static int placeVideoCall(Phone phone, String number, Uri contactRef, boolean bLoopbackCall) {
        int status = VIDEO_CALL_STATUS_DIALED;
        try {
            if (DBG) log("placeVideoCall: number=" + number + "  contactRef="+ contactRef + "  loopback="+bLoopbackCall);

            Connection cn = phone.videoDial(number, bLoopbackCall);
            if (DBG) log("===> phone.videoDial() returned cn: " + cn);

            // TODO: LGE, check if MMI has to be checked also for VIDEO CALL
            if (cn == null) {
                if (DBG) log("placeVideoCall: dialed MMI code: " + number);
                status = VIDEO_CALL_STATUS_DIALED_MMI;   
                // Set dialed MMI command to service
                if (mNwService != null) {
                    try {
                        mNwService.setMmiString(number);
                        if (DBG) log("placeVideoCall: Extended NW bindService setUssdString (" + number + ")");
                    } catch (RemoteException e) {
                        mNwService = null;
                    }
                }
                ...
                // TODO: LGE, AudioManager modes for VIDEO CALL
                setAudioMode(phone.getContext(), AudioManager.MODE_IN_CALL);
            }
        } catch (CallStateException ex) {
            Log.w(LOG_TAG, "PhoneUtils: Exception from phone.videoDial()", ex);
            status = VIDEO_CALL_STATUS_FAILED;
        }
    ...
    static CallerInfoToken startGetCallerInfo(Context context, Connection c,
        ...
        if (userDataObject instanceof Uri) {
            ...
            cit.currentInfo.cnapName =  c.getCnapName();
              /*  taesung.lee 
              // This can still get overwritten by ContactInfo later              
...
//20101109 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [START_LGE_LAB]
			cit.currentInfo.cdnipNumber = c.getCdnipNumber();
            Log.v(LOG_TAG,"cit.currentInfo.cdnipNumber    111     "+cit.currentInfo.cdnipNumber);
//20101109 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [END_LGE_LAB]
...
//20101117 yongjin.her@lge.com Phone Number full match : SKT need both full and min-match [START_LGE_LAB1]
                    if(StarConfig.COUNTRY.equals("KR") && StarConfig.OPERATOR.equals("SKT"))
                    {
                        if("true".equals(SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISROAMING)) 
                            && c!=null && c.isIncoming() )
                        {   // SKT incomming in Roaming Area is Min-Match. (It doesn't mean RAD, just Roaming Area.)
                            cit.asyncQuery = CallerInfoAsyncQuery.startQuery(QUERY_TOKEN, context,
                                    number, sCallerInfoQueryListener, c);
                        }
                        else
                        {   // SKT Other case is Full-Match
                            cit.asyncQuery = CallerInfoAsyncQuery.startQueryFullMatch(QUERY_TOKEN, context,
                                    number, sCallerInfoQueryListener, c);
                        }
                    }
                    else
                    if(StarConfig.COUNTRY.equals("KR")
                        && (StarConfig.OPERATOR.equals("LGT") || StarConfig.OPERATOR.equals("KT") ))
                    {   // Other Domestic Carrier is always Full-Match
                        cit.asyncQuery = CallerInfoAsyncQuery.startQueryFullMatch(QUERY_TOKEN, context,
                                number, sCallerInfoQueryListener, c);
                    }    
                    else
                    {   //Original Case : Maybe Not Korea
//20101117 yongjin.her@lge.com Phone Number full match : SKT need both full and min-match [END_LGE_LAB1]
                        cit.asyncQuery = CallerInfoAsyncQuery.startQuery(QUERY_TOKEN, context,
                                number, sCallerInfoQueryListener, c);
//20101117 yongjin.her@lge.com Phone Number full match : SKT need both full and min-match [START_LGE_LAB1]
                    }
...
//20101109 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [START_LGE_LAB]
					cit.currentInfo.cdnipNumber = c.getCdnipNumber();
		            Log.v(LOG_TAG,"cit.currentInfo.cdnipNumber    222     "+cit.currentInfo.cdnipNumber);
//20101109 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [END_LGE_LAB]
...
//20101109 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [START_LGE_LAB]
					cit.currentInfo.cdnipNumber = c.getCdnipNumber();
		            Log.v(LOG_TAG,"cit.currentInfo.cdnipNumber    333     "+cit.currentInfo.cdnipNumber);
//20101109 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [END_LGE_LAB]
...
if(cit!=null && cit.currentInfo!=null)
{
    if (DBG) log("PhoneUtils startGetCallerInfo() : cit.cnapName = " + cit.currentInfo.cnapName);
    if (DBG) log("PhoneUtils startGetCallerInfo() : cit.phoneNumber = " + cit.currentInfo.phoneNumber);
    if (DBG) log("PhoneUtils startGetCallerInfo() : cit.cdnipNumber = " + cit.currentInfo.cdnipNumber);
    if (DBG) log("PhoneUtils startGetCallerInfo() : cit.orignalNumber = " + cit.currentInfo.orignalNumber);
    if (DBG) log("PhoneUtils startGetCallerInfo() : cit.contactNumber = " + cit.currentInfo.contactNumber);
    if (DBG) log("PhoneUtils startGetCallerInfo() : cit.displayNumber = " + cit.currentInfo.displayNumber);
}
        return cit;
    }
...
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : display number [START_LGE_LAB] 
				if(StarConfig.COUNTRY.equals("KR")) {
                	PhoneCarrierUtils.setCallerInfoDisplayNumber(conn, ci);
				}
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : display number [END_LGE_LAB] 
...
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : display number [START_LGE_LAB] 
                        newCi.displayNumber = ci.displayNumber;
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : display number [END_LGE_LAB] 
                        ci = newCi;
                    }
                }

// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : special center number [START_LGE_LAB] 
				if(StarConfig.COUNTRY.equals("KR")) {
                	PhoneCarrierUtils.processSpecialCenterNumber(conn, ci);
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : special center number [END_LGE_LAB] 

// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : call feature code [START_LGE_LAB] 
                	PhoneCarrierUtils.processCallFeatureCode(conn, ci);
				}
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : call feature code [END_LGE_LAB] 
...
// 20101020 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call : roaming : process number : display number [START_LGE_LAB] 
                compactName = TextUtils.isEmpty(ci.displayNumber) ? ci.phoneNumber : ci.displayNumber;
                /**
                 * original
...
//20101110 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [START_LGE_LAB]
	static boolean isSecondNumber(Phone phone) {
        int phoneType = phone.getPhoneType();

        Call ringingCall = phone.getRingingCall();
        Connection conn;
        String cdnipNumber = null;

        if (phoneType == Phone.PHONE_TYPE_CDMA) {
           conn = ringingCall.getLatestConnection();
        } else {
           conn = ringingCall.getEarliestConnection();
        }

        if (conn != null) {
           cdnipNumber = conn.getCdnipNumber();
           if (DBG) log("isSecondNumber() : cdnipNumber = " + cdnipNumber);
        }

        if (!TextUtils.isEmpty(cdnipNumber)) {
           if (StarConfig.OPERATOR.equals("SKT")) {
              final String secondNumber = Settings.Secure.getString(phone.getContext().getContentResolver(), Settings.Secure.TELECOM_ANOTHER_NUMBER);
              if (DBG) log("isSecondNumber() : secondNumber = " + secondNumber);
              if ((secondNumber != null) && secondNumber.equals(cdnipNumber)) {
                 return true;
              } else {
                 return false;
              }
...
//20101110 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Second Number Display (SKT NumberPlus, KT TwoNumber, LGT DualNumber) [END_LGE_LAB]
    ...
    static void startNewCall(final Phone phone) {
        ...
        // LGE_UPDATE_S sanghoon.roh@lge.com 2010/06/11 do not tx mute when add call
        /*
        ...
        // LGE_UPDATE_E sanghoon.roh@lge.com 2010/06/11 do not tx mute when add call
...
//jongik2.kim 20101020 cp call path [start]
	static void changeModemPath(Context context, int path) {
        if (DBG) log("changeModemPath(path=" + path + ")...");
        AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
        
        if((path == 2) && audioManager.isWiredHeadPhoneOn())
			path = 5;
		
        audioManager.changeModemAudioPath(path);
		
    }

    static boolean isHeadsetOn(Context context) {
        AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
...
//jongik2.kim 20101020 cp call path [end]	
    ...
    static void turnOnHeadset(Context context, boolean flag, boolean store) {
        if (DBG) log("turnOnHeadset(flag=" + flag + ", store=" + store + ")...");
        AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
        if(sIsHeadsetEnabled != flag){
        audioManager.setWiredHeadsetOn(flag);
		    sIsHeadsetEnabled = flag;
        }
    }
    ...
    // [START] jaeyoung.ha@lge.com 2010.08.27 LAB1_CallUI if headset and speaker, headset's priority is high
    // LGE_CHANGE_S [shinhae.lee@lge.com] 2010-04-27 : if headset and speaker, headset's priority is high
    static boolean isSpeakerEnabled(Context context) {
    	AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
    	PhoneApp app = PhoneApp.getInstance();
    	BluetoothHandsfree bthf = app.getBluetoothHandsfree();
    	
    	if (audioManager.isWiredHeadsetOn() || ((bthf != null) && bthf.isAudioOn())) { // InCallScreen.isBluetoothAudioConnected()
    		return false;
    	} else {
    		return true;
    	}
    ...
    // [END] jaeyoung.ha@lge.com 2010.08.27 LAB1_CallUI if headset and speaker, headset's priority is high
	...
	//jongik2.kim 20100809 call_mute [start]	
        ...
        //} else {
            phone.setMute(muted);
        //}
        //jongik2.kim 20100809 call_mute [end]
        if (muted) {
    ...
    /* package */ static void setAudioMode(Context context, int mode) {
        ...
        if (!ignore) {
	     ...
	     if(PhoneApp.mIsVideoCall && (mode == AudioManager.MODE_IN_CALL))
		 mode = AudioManager.MODE_VT_CALL;			
        ...
        if (DBG) Log.d(LOG_TAG, "setAudioMode End");
    }
    	...
    	if (hasRingingCall) {
    		...
    		if (phoneType == Phone.PHONE_TYPE_CDMA) {
    			...
    			if (c != null && !PhoneNumberUtils.isEmergencyNumber(c.getAddress())) {
    				if (getMute(phone)) {
    					if (DBG) log("handleHeadsetHook: UNmuting...");
    					//setMute(phone, false); // sj.ahn Hook key -  instead mute of hangup
    					hangup(phone); // sj.ahn Hook key -  instead mute of hangup
    				} else {
    					if (DBG) log("handleHeadsetHook: muting...");
    					//setMute(phone, true);	// sj.ahn Hook key -  instead mute of hangup
    					hangup(phone); // sj.ahn Hook key -  instead mute of hangup
    				}
    			}
    ...
    //20101101 sumi920.kim@lge.com SKT Option menu [START_LGE_LAB1]
	// [2010.04.17] joy.park@lge.com, add to transfer call menu
    //START youngmi.uhm@lge.com 2010. 9. 02. LAB1_CallUI KT IOT err msg after transfer call
    static void setIsTransferCall(boolean value) {
    	mIsTransferCall = value;
    	//Log.d("panteon","setIsTransferCall:" + value);
    }
    static boolean getIsTransferCall() {
    	//Log.d("panteon","getIsTransferCall:" + mIsTransferCall);
    	return mIsTransferCall;
    }
    //END youngmi.uhm@lge.com 2010. 9. 02. LAB1_CallUI KT IOT err msg after transfer call
	static void transferCall(Phone phone) {
        try {
            if (DBG) log("transferCall");
            phone.explicitCallTransfer(true);
        } catch (CallStateException ex) {
            Log.w(LOG_TAG, "transferCall: caught " + ex, ex);
        }
    }
    //20101101 sumi920.kim@lge.com SKT Option menu [START_LGE_LAB1]
...
//<!--[sumi920.kim@lge.com] 2010.08.27	  LAB1_CallUI AutoCall Test [START] -->
	static void setAutoCallTest( boolean bSet, long connectionTime ){
		bAutoCallTest = bSet;
		mAutoConnectionTime = connectionTime;
		bAutoCallRunning = false;
	}

	static boolean isAutoCallTest(){
		return bAutoCallTest;
	}
	static long  	getAutoConnectionTime(){
		return mAutoConnectionTime;
	}

	static void setAutoCallRunning( boolean bSet )
	{
		bAutoCallRunning = bSet;
	}
	static boolean isAutoCallRunning(){
		return bAutoCallRunning;
	}
	
//<!--[sumi920.kim@lge.com] 2010.08.27	  LAB1_CallUI AutoCall Test [END] -->
//20101007 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Call Silent String [START_LGE_LAB]
    static void setIsReserveCall(boolean value) {
        mIsReserveCall = value;
    }

    static boolean getIsReserveCall() {
        return mIsReserveCall;
    }
//20101007 wonho.moon@lge.com <mailto:wonho.moon@lge.com> Call Silent String [END_LGE_LAB]
    //
    ...
    /* package */ public static void dumpCallState(Phone phone) {
        PhoneApp app = PhoneApp.getInstance();
        Log.d(LOG_TAG, "dumpCallState():");
        Log.d(LOG_TAG, "- Phone: " + phone + ", name = " + phone.getPhoneName()
              + ", state = " + phone.getState() + "  isVideocall = " + app.mIsVideoCall );  // Add Video Call
...
//<!--//20100916 sumi920.kim@lge.com InCall-Recording [START_LGE_LAB1]	 -->
    public static void setSoundRecording(boolean bSet)
    {
    	mIsSoundRecording = bSet;
    }
    public static boolean isSoundRecording()
    {
    	return mIsSoundRecording;
    }
    public static void setDuration(long duration)
    {
    	mRecDuration = duration;
    }
    public static long getRecDuration()
    {
    	return mRecDuration;
    }
    ...
    //20101105 sumi920.kim@lge.com SKT SKAF SpeakerOn/Off setting [START_LGE_LAB1]
    public static boolean haveToSetSKAFSpeakerMode()
    {
    	return bHaveToSKAFSpeakerMode;
    }
    public static void initSKAFSpeakerMode(boolean bSet,boolean bSpeakerMode)
    {
    	bHaveToSKAFSpeakerMode = bSet;
    	bSKAFSpeakerMode = bSpeakerMode;
    }
    public static boolean getSKAFSpeakerMode()
    {
    	return bSKAFSpeakerMode;
    }
    
    
    public static void haveToSetSKAFSpeakerMode(boolean bSet, boolean bSpeakerMode)
    {
    	Log.d(LOG_TAG, "haveToSetSKAFSpeakerMode ...... ");
    	Log.d(LOG_TAG, "bHaveToSKAFSpeakerMode : " + bSet );
    	Log.d(LOG_TAG, "bSKAFSpeakerMode : " + bSpeakerMode );
    	bHaveToSKAFSpeakerMode = bSet;
    	bSKAFSpeakerMode = bSpeakerMode;
    }
    //20101105 sumi920.kim@lge.com SKT SKAF SpeakerOn/Off setting [END_LGE_LAB1]
    ...
    static boolean getIsNoSaveCallDB(Phone phone) {
    	...
    	if ("1".equals(SystemProperties.get(TelephonyProperties.PROPERTY_TEST_CALL))) {
    		if (DBG) log("getIsNoSaveCallDB() : test mode = " + SystemProperties.get(TelephonyProperties.PROPERTY_TEST_CALL));
    		return true;
    			// LGE_CHANGE_E [yongjin.her@lge.com] 2010-04-01 : not add test call
    	} else {
    			//START jahyun.park@lge.com 2010.10.01 TD : 43419
    		if (phone.getPhoneName().equals("CDMA")) {
    			String phoneNumber = phone.getLine1Number();
    			if (TextUtils.isEmpty(phoneNumber) || phoneNumber.startsWith("0000000000") || phoneNumber.startsWith("0100000000")) { // if (phoneNumber.equals("0000000000") || phoneNumber.equals("0100000000")) {
    				if (DBG) log("getIsNoSaveCallDB() : phoneNumber = " + phoneNumber);
    				return true;
    			}
    		}
    		//END jahyun.park@lge.com 2010.10.01 TD : 43419
    		// TODO
    	}
    	return false;
    }
    ...
    //20101111 sumi920.kim@lge.com Thunder porting [START_LGE_LAB1]
    // LGE_UPDATE_S sanghoon.roh@lge.com 2010/06/21 handle LG headsethook key scenario
    /* package */ static boolean handleHeadsetHookLong(Phone phone) {
    	if (DBG) log("handleHeadsetHookLong()...");
    	// If the phone is totally idle, we ignore HEADSETHOOK events
    	// (and instead let them fall through to the media player.)
    	if (phone.getState() == Phone.State.IDLE) {
    		return false;
    	...
    	if (hasRingingCall) {
    		...
    		if (phoneType == Phone.PHONE_TYPE_CDMA) {
    			...
    			if (hasActiveCall) {
    				if (DBG) log("handleHeadsetHook: ringing (active call) ==> answer!");
    				answerAndEndActive(phone);
    			} else {
    				if (DBG) log("handleHeadsetHook: ringing ==> hangup!");
    				hangup(phone);
    			}
    		} else {
    			throw new IllegalStateException("Unexpected phone type: " + phoneType);
    ...
    //20101111 sumi920.kim@lge.com Thunder porting [END_LGE_LAB1]
    //20101111 sumi920.kim@lge.com Receive HDVideoCall Enable Event From HDVideoCall App. [START_LGE_LAB1]	 -->
	static void setHDVideoCallStatus(boolean bHDAvailable) {
		// TODO Auto-generated method stub
		mHDVisibilty = bHDAvailable;
	}
	static boolean isHDVideoCallAvailable(){

		return mHDVisibilty;
	}

	//20101111 sumi920.kim@lge.com Receive HDVideoCall Enable Event From HDVideoCall App. [END_LGE_LAB1]	 -->
}
