...
// 20101020 seunghwan.moon@lge.com, Add VT Call Icon.[START_LGE_LAB1]
import android.os.ServiceManager;
import com.android.internal.telephony.ITelephony;
import android.os.RemoteException;

import com.lge.config.StarConfig;
// 20101020 seunghwan.moon@lge.com, Add VT Call Icon.[END_LGE_LAB1]
...
public class NotificationMgr implements CallerInfoAsyncQuery.OnQueryCompleteListener{
    ...
    void notifyMissedCall(String name, String number, String label, long date) {
        ...
        //ANDY_PORTING LockScreen [raya.lee@lge.com 100513] LockScreen Event Handling
        final Intent addMissedCalls  = new Intent(com.lge.content.Andy_Intent.ACTION_MISSED_CALLS);
        addMissedCalls.putExtra("pkg", com.lge.content.Andy_ILockEvent.MSG_MISSED_CALLS);
        addMissedCalls.putExtra("number", Integer.toString(mNumberMissedCalls));
        mContext.sendBroadcast(addMissedCalls);
        //ANDY_END
...
//ANDY_PORTING LockScreen [raya.lee@lge.com 100513] LockScreen Event Handling
	Intent delMissedCalls  = new Intent(com.lge.content.Andy_Intent.ACTION_MISSED_CALLS);
        delMissedCalls.putExtra("number", Integer.toString(0));
        delMissedCalls.putExtra("pkg", com.lge.content.Andy_ILockEvent.MSG_MISSED_CALLS);
        mContext.sendBroadcast(delMissedCalls);
//ANDY_END
...
// 20101020 seunghwan.moon@lge.com, Add VT Call Icon.[START_LGE_LAB1]
                if (StarConfig.COUNTRY.equals("KR"))
                {
                    ...
                    if(phone != null)
                    {
                        ...
                        try 
                        {
                            isVideoCall = phone.isVideoCall();
                        } catch (RemoteException e)
                        {
                            if (DBG) log("VT RemoteException");
                        }
                        
                        if (isVideoCall)
                        {
                            resId = com.lge.internal.R.drawable.stat_sys_phone_vt;
                        }
                        else
                        {
...
//20100818 yongwoon.choi@lge.com Call End Button in Expanded Status Bar [START_LGE]
        Intent btnIntent = new Intent("com.lge.phone.action.CALL_END");
        notification.btnIntent = PendingIntent.getBroadcast(mContext, 0, btnIntent, 0);
        notification.btnText = mContext.getString(R.string.menu_endCall);
//20100818 yongwoon.choi@lge.com Call End Button in Expanded Status Bar [END_LGE]
        ...
        long chronometerBaseTime = 0; //20100929 Call duration time display in HomeScreen
        if (currentConn != null) {
            ...
            chronometerBaseTime = SystemClock.elapsedRealtime() - callDurationMsec; //20100929 Call duration time display in HomeScreen
        ...
        mNotificationMgr.notify(IN_CALL_NOTIFICATION,
                                notification);
//20100929 Call duration time display in HomeScreen[Start] yjyj.kim@lge.com
		Intent intent = new Intent("com.lge.launcher.ACTION_INCALLTIMERSTART");
		String baseTime = mContext.getResources().getString(R.string.current_time);
		intent.putExtra(baseTime, chronometerBaseTime);

		//  20101012 jinsun.shim@lge.com current call state for HomeScreen display [START]
		final boolean isDialing = mPhone.getForegroundCall().isDialingOrAlerting();
		String currentState = mContext.getResources().getString(R.string.current_call_state);
		if( isDialing )
			intent.putExtra(currentState, Call.State.DIALING.toString());
		else if ( hasActiveCall )
			intent.putExtra(currentState, Call.State.ACTIVE.toString());
		else if(hasHoldingCall )
			intent.putExtra(currentState, Call.State.HOLDING.toString());
		else
			intent.putExtra(currentState, Call.State.IDLE.toString());
		Log.i("Noti call", "isDialing = " + isDialing + "hasActiveCall = "+hasActiveCall + " hasHoldingCall "+hasHoldingCall);
		//  20101012 jinsun.shim@lge.com current call state for HomeScreen display [END]

		mContext.sendBroadcast(intent);
//20100929 Call duration time display in HomeScreen[End]
...
// 20100508 doohwan.kim@lge.com VoiceMailLockScreen
			int vmCount = 0;
            ...
            if (mPhone.getPhoneType() == Phone.PHONE_TYPE_CDMA) {
                vmCount = mPhone.getVoiceMessageCount();
                String titleFormat = mContext.getString(R.string.notification_voicemail_title_count);
                notificationTitle = String.format(titleFormat, vmCount);
            }else{
                 vmCount = mPhone.getVoiceMessageCount();	// 20100508 doohwan.kim@lge.com VoiceMailLockScreen
// LGE_VOICEMAIL START
//display number of received voicemail messages
                  if (vmCount > 1){
                        String titleFormat = mContext.getString(R.string.notification_voicemail_title_count);
                        notificationTitle = String.format(titleFormat, vmCount);
                  }
