...
//LGE_PHONEBOOK_EXTENSION START
    ...
    protected static final String WHERE = null;
    
    protected static final int NAME_COLUMN = 0;
    protected static final int NUMBER_COLUMN = 1;
    protected static final int NUMBER2_COLUMN = 2;
...
//LGE_PHONEBOOK_EXTENSION START
    ...
    protected Uri resolveIntent() {
        ...
        if (intent.getData() == null) {
            intent.setData(Uri.parse("content://icc/phonebook"));
        }
...
//LGE_PHONEBOOK_EXTENSION START
    ...
    protected int getCheckedItems() {
        int size = getCurrentAdapter().getCount();
        SparseBooleanArray spa = getListView().getCheckedItemPositions();
        int k = 0;
    ...
    protected int[] getPositions() {
        int size = getCurrentAdapter().getCount();
        int[] positionsArray = new int[getCheckedItems()];
        ListView lv = getListView();
        SparseBooleanArray spa = lv.getCheckedItemPositions();
        int k = 0;
        for (int i = 0; i < size && k < positionsArray.length; i++) {
            if (spa.get(i)) {
                positionsArray[k++] = i;
            }
        }
    ...
    /**
     * Returns current list adapter as BaseListAdapter.
     * 
     * @return current list adapter as BaseListAdapter.
     */
    protected final SimpleCursorAdapter getCurrentAdapter() {
