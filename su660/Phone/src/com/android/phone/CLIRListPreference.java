...
// LGE_CUSTOMER_SERVICE_PROFILE START
        if ((findIndexOfValue(getValue()) != prevValue) && !isCLIRItemAccessible(findIndexOfValue(getValue()))) {
            showToast(R.string.service_not_accessible);
            return;
        }
// LGE_CUSTOMER_SERVICE_PROFILE END
...
// LGE_CUSTOMER_SERVICE_PROFILE START
    private boolean isCLIRItemAccessible(int item) {
        boolean accessible = true;
        // LGE_UPDATE_S // 20100807 jungyeon.kim@lge.com   // prevent CLIR control using CSP
        /*
        switch (item) {
            case CommandsInterface.CLIR_SUPPRESSION:
                accessible = phone.isServiceCustomerAccessible(CspConstants.GROUPCODE_NUMBER_IDENTIF, CspConstants.SERVICE_CLI_SEND);
                break;
            case CommandsInterface.CLIR_INVOCATION:
                accessible = phone.isServiceCustomerAccessible(CspConstants.GROUPCODE_NUMBER_IDENTIF, CspConstants.SERVICE_CLI_BLOCK);
                break;
        }
        // LGE_UPDATE_E
        */
