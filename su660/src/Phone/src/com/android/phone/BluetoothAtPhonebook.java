...
//LG_BTUI
    ...
    public String getLastDialledNumber() {
        ...
        Cursor cursor = mContext.getContentResolver().query(Calls.CONTENT_URI, projection,
                ...
                " LIMIT 1");
//LG_BTUI
        if(cursor != null) // E720_LGE_UPDATE dkmoon 20100726 add exception handling.
        {
            try{
...
//LG_BTUI
                if(LG_BTUI_CDMA) {
                    return new AtCommandResult("+CPBS: (\"ME\",\"DC\",\"RC\",\"MC\")");
                } else
//LG_BTUI
...
//+++ BRCM
                ...
                if (index2 > pbr.cursor.getCount()) {
                	index2 = pbr.cursor.getCount();
                }
                /* Google Original
                if (pbr.cursor.getCount() == 0 || index1 <= 0 || index2 < index1  ||
