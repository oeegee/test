...
public class DTMFTwelveKeyDialer implements
        ...
        View.OnKeyListener,
//20101026 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call ui : send/delete btn on InCallScreen dialpad [START_LGE_LAB]
        View.OnLongClickListener,
        View.OnClickListener{
//20101026 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call ui : send/delete btn on InCallScreen dialpad [END_LGE_LAB]
...
//20101004 yusei.koh@lge.com <mailto:yusei.koh@lge.com> call ui : send/delete btn on InCallScreen dialpad [START_LGE_LAB]
    	if(StarConfig.OPERATOR.equals("SKT")){
            mDeleteButton = (Button) mDialerView.findViewById(R.id.deleteDigit);
            if (mDeleteButton != null) {
               if (mInCallScreen.isDialpadSendKeyEnabled()) {
...
//20101101 sh80.choi@lge.com DTMF Tone Tuning [START_LGE_LAB1]
						if (mAudioManager.isWiredHeadsetOn()) {
                        	mToneGenerator = new ToneGenerator(AudioManager.STREAM_DTMF, 75);
						} else {
//20101101 sh80.choi@lge.com DTMF Tone Tuning [END_LGE_LAB1]
...
//20101026 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call ui : send/delete btn on InCallScreen dialpad [START_LGE_LAB]
		...
		/*
		else if (StarConfig.OPERATOR.equals("SKT") && (viewId == R.id.deleteDigit) && (event.getAction() == MotionEvent.ACTION_UP)) {
				deleteDigit();
		}
		*/
//20101026 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call ui : send/delete btn on InCallScreen dialpad [END_LGE_LAB]
...
//20101026 wonho.moon@lge.com <mailto:wonho.moon@lge.com> call ui : send/delete btn on InCallScreen dialpad [START_LGE_LAB]
	public void onClick(View v){
		int id = v.getId();
		switch (id) {
            case R.id.deleteDigit: { // deleteButton->btnBackspac
	            deleteDigit();
				break;
            }
        }
	...
	public boolean onLongClick(View v){
		final Editable digits = mDialpadDigits.getText();
		int id = v.getId();
        switch (id) {
            case R.id.deleteDigit: { // deleteButton->btnBackspac
                digits.clear();
...
//20101004 yusei.koh@lge.com <mailto:yusei.koh@lge.com> call ui : send/delete btn on InCallScreen dialpad [START_LGE_LAB]
    public void deleteDigit() {
        if (DBG) log("deleteDigit()...");

        if (mDialpadDigits != null) {
            Editable digits = mDialpadDigits.getText();
            int length = digits.length();
            if(length > 0)
                digits.delete(length-1, length);
        }
    }

    public String getDigits() {
        if (mDialpadDigits != null) {
            return mDialpadDigits.getText().toString();
        }
